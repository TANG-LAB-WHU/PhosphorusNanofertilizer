...
def print_graph_summary(G):
    ...
    print("\n" + "="*50)
    print("   Phosphorus Nanofertilizer KNOWLEDGE GRAPH SUMMARY")
    print("="*50)
...
def visualize_matplotlib(G, output_path, max_label_length=25):
    ...
    try:
        ...
        nx.draw_networkx_nodes(G, pos, node_size=node_sizes, node_color=colors, 
                               alpha=0.85, ax=ax, edgecolors='#333333', linewidths=2)
        
        # Draw short code labels directly on nodes
        nx.draw_networkx_labels(G, pos, labels=short_labels, font_size=12, 
                               font_weight='bold', ax=ax, font_color='#222222')
        ...
        ax.legend(handles=legend_handles, loc='upper left', fontsize=10, 
                 title='Node Types', title_fontsize=11, framealpha=0.95)
        
        plt.title("Phosphorus Nanofertilizer Knowledge Graph", fontsize=18, fontweight='bold', pad=15)
        ax.axis("off")
        ...
        table_text = "\n".join(table_lines)
        ax_table.text(0.0, 1.0, table_text, fontsize=9, fontfamily='monospace',
        ...
        print(f"     Figure size: {fig_size}x{fig_size+4}, Nodes: {num_nodes}, Edges: {G.number_of_edges()}")
        return True
