...
Handles LCI data structures and calculations for nano hydroxyapatite phosphorus fertilizer.
"""
...
class LifeCycleInventory:
    ...
    def get_mass_balance(self) -> Dict[str, float]:
        ...
        # Sum inputs (kg only)
        total_inputs = sum(
            f.quantity for f in self.inputs
            if f.unit == "kg"
        )
        
        # Sum outputs and emissions
        total_outputs = sum(
            f.quantity for f in self.outputs
            if f.unit == "kg"
        )
        
        total_emissions = sum(
            f.quantity for f in self.emissions_air + self.emissions_water + self.emissions_soil
            if f.unit == "kg"
        )
        ...
        return {
            ...
            "balance_pct": ((total_inputs - total_outputs - total_emissions) / total_inputs * 100) if total_inputs > 0 else 0
        }


if __name__ == "__main__":
    # Example usage for NanoP synthesis
    lci = LifeCycleInventory(
        process_name="NanoP Synthesis",
        functional_unit="1 tonne NanoP",
        functional_unit_value=1000
    )
    
    # Add inputs
    lci.add_input("CaCl2", 650, "kg")
    lci.add_input("H3PO4", 580, "kg")
    lci.add_input("NH4OH", 120, "kg")
    lci.add_input("Deionized water", 5000, "kg")
    lci.add_input("Electricity", 800, "kWh")
    
    # Add outputs
    lci.add_output("NanoP", 1000, "kg")
    
    # Add emissions
    lci.add_emission("CO2", 380, "kg", "air")
    lci.add_emission("NH3", 5, "kg", "air")
